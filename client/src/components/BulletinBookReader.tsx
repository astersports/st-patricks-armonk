...
export default function BulletinBookReader({ pdfUrl, title, onClose }: BulletinBookReaderProps) {
  ...
  useEffect(() => {
    const updateWidth = () => {
      if (isFullscreen) {
        // In fullscreen, use viewport width minus nav controls
        setPageWidth(Math.min(window.innerWidth - 32, 900));
      } else if (containerRef.current) {
        // Inline: fill the container width with some padding
  ...
  // PDF page component
  const PDFPage = ({ width }: { width: number }) => (
    <div
  ...
  if (isFullscreen) {
    return (
      <div className="fixed inset-0 z-[100] bg-neutral-900 flex flex-col">
        {/* Top toolbar */}
        <div className="flex items-center justify-between px-3 sm:px-4 py-2 bg-black/60 backdrop-blur-sm border-b border-white/10 shrink-0">
        ...
        {/* Main reading area */}
        <div className="flex-1 overflow-auto flex items-start justify-center py-4 px-4">
          <Document
            file={pdfUrl}
          ...
          >
            {numPages > 0 && <PDFPage width={pageWidth} />}
          </Document>
        </div>

        {/* Bottom nav */}
        <div className="shrink-0 bg-black/60 backdrop-blur-sm border-t border-white/10 px-4 py-2">
          <NavControls dark />
        </div>
      </div>
    );
  ...
  // Inline (non-fullscreen) view — full-width single page
  return (
    <div ref={containerRef} className="relative w-full bg-stone-50 rounded-xl overflow-hidden border border-border/40">
      ...
      {/* Fullscreen hint on mobile */}
      <div className="sm:hidden text-center pb-2">
        <button onClick={toggleFullscreen} className="text-xs text-primary font-medium hover:underline">
          Tap for full screen reading
        </button>
      </div>
    </div>
  );
